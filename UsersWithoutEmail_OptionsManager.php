...
class UsersWithoutEmail_OptionsManager {
  ...
  public function settingsPage() {
    if (!current_user_can('manage_options')) {
      wp_die(__('You do not have sufficient permissions to access this page.', 'users-without-email'));
    }
    ...
    ?>
    <div class="wrap">
      <h2><?php _e('System Settings', 'users-without-email'); ?></h2>
      <table class="form-table"><tbody>
      <tr><td><?php _e('System', 'users-without-email'); ?></td><td><?php echo php_uname(); ?></td></tr>
      <tr><td><?php _e('PHP Version', 'users-without-email'); ?></td>
        <td><?php echo phpversion(); ?>
        <?php
        if (version_compare('5.2', phpversion()) > 0) {
          echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
          _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'users-without-email');
          echo '</span>';
        }
        ?>
        </td>
      </tr>
      <tr><td><?php _e('MySQL Version', 'users-without-email'); ?></td>
        <td><?php echo $this->getMySqlVersion() ?>
          <?php
          echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
          if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
            _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'users-without-email');
          }
          echo '</span>';
          ?>
        </td>
      ...
      <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'users-without-email'); ?></h2>
      ...
      <?php settings_fields($settingsGroup); ?>
        ...
        <p class="submit">
          <input type="submit" class="button-primary"
               value="<?php _e('Save Changes', 'users-without-email') ?>"/>
        </p>
      </form>
    </div>
    <?php
  ...
  protected function getOptionValueI18nString($optionValue) {
    switch ($optionValue) {
      case 'true':
        return __('true', 'users-without-email');
      case 'false':
        return __('false', 'users-without-email');

      case 'Administrator':
        return __('Administrator', 'users-without-email');
      case 'Editor':
        return __('Editor', 'users-without-email');
      case 'Author':
        return __('Author', 'users-without-email');
      case 'Contributor':
        return __('Contributor', 'users-without-email');
      case 'Subscriber':
        return __('Subscriber', 'users-without-email');
      case 'Anyone':
        return __('Anyone', 'users-without-email');
    }
    return $optionValue;
  }
